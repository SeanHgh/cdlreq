...
class TraceabilityMatrixExporter:
    ...
    def __init__(
        self, requirements: List[Requirement], specifications: List[Specification]
    ):
    ...
    def _create_specifications_sheet(self, workbook: openpyxl.Workbook) -> None:
        ...
        for row, spec in enumerate(self.specifications, 2):
            ...
            ws.cell(row=row, column=7, value=spec.design_notes or "")
            ws.cell(row=row, column=8, value=", ".join(spec.dependencies))
    ...
    def _create_traceability_matrix_sheet(self, workbook: openpyxl.Workbook) -> None:
        ...
        # Title
        ws.cell(
            row=1, column=1, value="Requirements to Specifications Traceability Matrix"
        )
    ...
    def _create_summary_sheet(self, workbook: openpyxl.Workbook) -> None:
        ...
        ws.cell(
            row=len(req_by_type) + 13,
            column=1,
            value="Requirements with Specifications:",
        )
        ws.cell(row=len(req_by_type) + 13, column=2, value=len(traced_reqs))

        ws.cell(
            row=len(req_by_type) + 14,
            column=1,
            value="Requirements without Specifications:",
        )
